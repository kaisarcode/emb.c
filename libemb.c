...
kc_emb_t *kc_emb_open(void) {
    ...
    GetTempFileNameA(temp_path, "gguf", 0, temp_file);
    f = fopen(temp_file, "w+b");
    if (f) {
        fwrite(model_gguf, 1, model_gguf_len, f);
        rewind(f);
    }
#else
    f = fmemopen(model_gguf, model_gguf_len, "rb");
#endif
    ...
    for (int i = 0; i < n_tensors; i++) {
        ...
        if (t) {
            t->data = (void *)(model_gguf + data_offset + gguf_get_tensor_offset(ctx->gguf, i));
        }
    }

    int k;
    k = gguf_find_key(ctx->gguf, "bert.block_count");
    if (k >= 0) ctx->n_layer = gguf_get_val_u32(ctx->gguf, k);

    k = gguf_find_key(ctx->gguf, "bert.embedding_length");
    if (k >= 0) ctx->n_embd = gguf_get_val_u32(ctx->gguf, k);

    k = gguf_find_key(ctx->gguf, "bert.attention.head_count");
    if (k >= 0) ctx->n_head = gguf_get_val_u32(ctx->gguf, k);

    k = gguf_find_key(ctx->gguf, "bert.attention.layer_norm_epsilon");
    if (k >= 0) ctx->layer_norm_eps = gguf_get_val_f32(ctx->gguf, k);
    else ctx->layer_norm_eps = 1e-12f;

    k = gguf_find_key(ctx->gguf, "tokenizer.ggml.cls_token_id");
    if (k >= 0) ctx->cls_token_id = gguf_get_val_u32(ctx->gguf, k);
    else ctx->cls_token_id = 101;

    k = gguf_find_key(ctx->gguf, "tokenizer.ggml.seperator_token_id");
    if (k >= 0) ctx->sep_token_id = gguf_get_val_u32(ctx->gguf, k);
    else ctx->sep_token_id = 102;

    k = gguf_find_key(ctx->gguf, "tokenizer.ggml.padding_token_id");
    if (k >= 0) ctx->pad_token_id = gguf_get_val_u32(ctx->gguf, k);
    else ctx->pad_token_id = 0;

    k = gguf_find_key(ctx->gguf, "tokenizer.ggml.unknown_token_id");
    if (k >= 0) ctx->unk_token_id = gguf_get_val_u32(ctx->gguf, k);
    else ctx->unk_token_id = 100;

    k = gguf_find_key(ctx->gguf, "tokenizer.ggml.tokens");
    if (k >= 0) {
        ctx->n_vocab = gguf_get_arr_n(ctx->gguf, k);
        ctx->vocab = (char **)calloc(ctx->n_vocab, sizeof(char *));
        for (int i = 0; i < ctx->n_vocab; i++) {
            const char *tok = gguf_get_arr_str(ctx->gguf, k, i);
            ctx->vocab[i] = strdup(tok);
        }
    }
...
void kc_emb_close(kc_emb_t *ctx) {
    ...
    if (ctx->vocab_hash) free(ctx->vocab_hash);
...
static void wordpiece_tokenize(kc_emb_t *ctx, const char *input, int *tokens, int *n_tokens) {
    *n_tokens = 0;
    ...
    while (i < len && *n_tokens < MAX_TOKENS - 1) {
        ...
        for (int k = 0; k < word_len; k++) word[k] = tolower(input[i + k]);
        word[word_len] = '\0';
...
int kc_emb_exec(kc_emb_t *ctx, const char *input) {
    ...
    float *pool = (float *)malloc(ctx->n_embd * sizeof(float));
    if (!pool) {
    ...
    for (int i = 0; i < ctx->n_embd; i++) pool[i] = 0.0f;
    for (int t = 0; t < n_tokens; t++) {
        for (int i = 0; i < ctx->n_embd; i++) {
            pool[i] += out_data[t * ctx->n_embd + i];
        }
    }
    for (int i = 0; i < ctx->n_embd; i++) {
        pool[i] /= (float)n_tokens;
        printf("%f%s", pool[i], i == ctx->n_embd - 1 ? "" : " ");
    }
    printf("\n");
